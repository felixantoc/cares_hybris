 ...
 *  
 */
package de.hybris.assuramed.cockpits.setup;
...
import de.hybris.assuramed.cockpits.constants.AssuramedCockpitsConstants;
 ...
 */
@SystemSetup(extension = AssuramedCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(AssuramedCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(AssuramedCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						AssuramedCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/assuramedcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
